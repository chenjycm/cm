...
$(function(){        //根据点击的页面按钮，读取第几页面，并修改页码格式
		 $(document).on('click', '.pages',function(){
			...
			if (!$self.hasClass('active')) {
			$self.addClass('active').siblings().removeClass('active');
			var cur_page= $self.val();  console.log('当前页：'+cur_page);
	...
	$.ajax({
	        ...
	        success: function(res){
	            console.log(res);    //res就是一个对象数组，这里你就可以操作他了
			 	...
			 	var total_pages = res.data.totalPage; console.log(total_pages);
...
function setpages(tp,cp){				//自动设置分页按钮，最多显示6个数字页码
	...
	get_pages.push("<input class='pagespre' type='button' value='上一页'>");
	
	if(tp<7){						//小于7的情况：1/2/3/4/5/6按钮全部输出
		for(var i=1;i<=tp;i++){
			if(i==cp){
	...
	get_pages.push("<input class='pagesnext' type='button' value='下一页'>");
	if(cp==1){
		get_pages.shift();
	}
	if(cp==tp){
